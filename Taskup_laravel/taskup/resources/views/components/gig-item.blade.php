@props(['gig', 'currency_symbol','is_save_item', 'user_role', 'address_format' => 'city_state_country'])
            ...
            <div class="tk-icondetails">
                ...
                <ul class="tk-rateviews tk-rateviews2">
                    ...
                    @if($user_role == 'buyer' || Auth::guest())
                        <li>
                            <span class="tk-save-btn tk-fav-item {{$gig?->is_favourite || $is_save_item ? 'tk-favourite' : '' }}" wire:click.prevent="saveItem({{$gig->id}})">
                                <i class="icon-heart"></i> 
                                <em>{{$gig?->is_favourite || $is_save_item ? __('general.saved') : __('general.save')}}</em>
                            </span>
                        </li>
                    @endif
