...
class PageBuilderController extends Controller {
            ...
            if (file_exists(resource_path('views/pagebuilder/' . $directory . '/settings.php'))) {
                ...
                if (!empty($currentSettings['id']) && $directory == $currentSettings['id']) {
                    ...
                    $components[$currentSettings['id']] = array(
                        ...
                        'template' => view()->exists('pagebuilder.' . $directory . '.view') ? view('pagebuilder.' . $directory . '.view')->render() : view('pagebuilder::components.no-view')->render(),
                    );
    ...
    public function renderPage($pageSlug) {
        ...
        $pageSections = view('pagebuilder::components.pb-page-components', compact('page'));
        setCurrentPageId($page->id);
    ...
    public function getPageSectionHtml() {
        setCurrentPageId(request()->get('page_id'));
        return $this->getSectionHtml(request()->get('component_id'),request()->get('section_id'));
    }
}
